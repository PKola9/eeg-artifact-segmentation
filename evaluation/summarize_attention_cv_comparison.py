"""
Summarize the post-thesis attention integration experiment.

This summary intentionally uses threshold-only results:
    - random Split U-Net
    - M3 flow-initialized Split U-Net
    - attention-bottleneck Split U-Net initialized from the same M3 flow checkpoint

Smoothing/post-processing results are not included here.
"""
...
METHODS = [
    ...
    {
        "method": "attention_flowinit_split_unet",
        "experiment_prefix": "CV_ATTENTION_FLOWINIT",
        "description": "Split U-Net with Transformer bottleneck initialized from the same M3 flow checkpoint",
    },
]
...
def main() -> None:
    output_dir = OUTPUT_ROOT / "attention_cv_summary"
    output_dir.mkdir(parents=True, exist_ok=True)

    rows = []
    for method in METHODS:
        for fold_name, subjects in FOLDS:
            rows.append(threshold_row(method, fold_name, subjects))

    summary_rows = make_method_summary(rows)

    per_fold_path = output_dir / "attention_cv_per_fold_results.csv"
    summary_path = output_dir / "attention_cv_mean_std_summary.csv"
    ...
    for row in summary_rows:
        print(
            ...
            f"test Dice mean={row['test_dice_mean']}, std={row['test_dice_std']}"
        )
